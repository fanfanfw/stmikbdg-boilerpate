...
export default function AdminArchive() {
    ...
    const fileColumns = [
        ...
        { field: 'source_type', headerName: 'Jenis Arsip', width: 300, renderCell: (params) => params.row.row_type === 'file' ? <div className="flex items-center gap-1 whitespace-nowrap"><Chip label={archiveSourceLabel(params.row.source_type)} size="small" variant="outlined" />{['personal', 'admin_upload'].includes(params.row.source_type) && (params.row.request_file || params.row.requestFile) && <Chip label="Dipakai di Permintaan" size="small" color="warning" variant="outlined" />}</div> : '-' },
