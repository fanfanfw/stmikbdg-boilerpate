...
export default function AdminDistributions() {
    ...
    const loadDistributions = async () => {
        ...
        setError('');
        try {
            const response = await arsipApi.distributions({ ...filters, per_page: 50 });
    ...
    const loadRecipients = async (distribution = selected) => {
        if (!distribution) return;
        setRecipientsLoading(true);
        try {
            const response = await arsipApi.distributionRecipients(distributionId(distribution), { per_page: 50 });
    ...
    const openEdit = (row) => {
        ...
        setTargetPayload(normalizeTarget(row));
        setPreview(null);
        setDialogOpen(true);
    };

    const buildPayload = () => ({ title: form.title, description: form.description || null, ...targetPayload });
    ...
    const selectDistribution = async (row) => {
        ...
        setBulkJob(null);
        setBulkFile(null);
        await Promise.all([loadRecipients(row), loadJobs(row)]);
    };
    ...
    return (
        <div className="font-jakarta">
            ...
            <section className="bg-white rounded-lg border border-zinc-200 p-4 mb-4 flex flex-wrap gap-3"><TextField size="small" label="Cari" value={filters.search} onChange={(event) => setFilters((prev) => ({ ...prev, search: event.target.value }))} sx={{ minWidth: 220 }} /><TextField size="small" select label="Status" value={filters.status} onChange={(event) => setFilters((prev) => ({ ...prev, status: event.target.value }))} sx={{ minWidth: 150 }}><MenuItem value="">Semua</MenuItem><MenuItem value="draft">Draft</MenuItem><MenuItem value="published">Published</MenuItem><MenuItem value="archived">Archived</MenuItem></TextField><TextField size="small" select label="Target" value={filters.target_role} onChange={(event) => setFilters((prev) => ({ ...prev, target_role: event.target.value }))} sx={{ minWidth: 150 }}><MenuItem value="">Semua</MenuItem><MenuItem value="mahasiswa">Mahasiswa</MenuItem><MenuItem value="dosen">Dosen</MenuItem></TextField><Button variant="contained" startIcon={<SearchOutlined />} onClick={loadDistributions} sx={{ ...buttonSx, backgroundColor: '#2563eb' }}>Terapkan Filter</Button></section>
            {!meta && rows.length > 0 && <Alert severity="info" sx={{ mb: 2, borderRadius: '0.5rem' }}>Backend pagination distribusi belum tersedia. Data dipaginasi di browser.</Alert>}
            <section className="bg-white rounded-lg border border-zinc-200 mb-4"><CustomDataTable rows={rows} columns={columns} loading={loading} getRowId={(row) => distributionId(row)} pageSize={25} pageSizeOptions={[25, 50]} /></section>
            {selected && <section className="bg-white rounded-lg border border-zinc-200 p-4 mb-4"><div className="flex items-center justify-between gap-3 flex-wrap mb-3"><div><h2 className="text-sm font-semibold text-zinc-800">Penerima Distribusi</h2><p className="text-xs text-zinc-500">{selected.title} · {selected.status}</p></div><Button variant="outlined" onClick={() => Promise.all([loadRecipients(), loadJobs()])} sx={{ ...buttonSx, borderColor: '#e4e4e7', color: '#3f3f46' }}>Refresh Detail</Button></div>{!recipientsMeta && recipients.length > 0 && <Alert severity="info" sx={{ mb: 2, borderRadius: '0.5rem' }}>Backend pagination penerima belum tersedia. Data dipaginasi di browser.</Alert>}<CustomDataTable rows={recipients} columns={recipientColumns} loading={recipientsLoading} getRowId={(row) => recipientId(row)} pageSize={25} pageSizeOptions={[25, 50]} /></section>}
