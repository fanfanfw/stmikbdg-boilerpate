...
function canUploadRecipient(distribution, recipient) {
    const policy = recipient?.can_upload_file ?? recipient?.can_upload ?? distribution?.can_upload_files ?? distribution?.can_upload;
    return policy === undefined ? ['draft', 'published'].includes(distribution?.status) : Boolean(policy);
}
...
export default function AdminDistributions() {
    ...
    const loadDistributions = async (nextFilters = appliedFiltersRef.current) => {
        ...
        try {
            ...
            if (selectedId) {
                const refreshedSelection = unwrapped.data.find((item) => distributionId(item) === selectedId) || null;
    ...
    const createCorrection = async (row) => {
        ...
        try {
            ...
            customSwal.toast.success({ message: 'Draft koreksi berhasil dibuat.' });
            await loadDistributions({ ...appliedFiltersRef.current, status: 'draft', page: 1 });
            if (distributionId(correction)) {
    ...
    const columns = [
        ...
        { field: 'actions', headerName: 'Aksi', width: 410, sortable: false, renderCell: (params) => <div className="flex gap-1 flex-wrap"><Button size="small" onClick={() => selectDistribution(params.row)} sx={buttonSx}>Detail</Button>{params.row.status === 'draft' && <Button size="small" onClick={() => openEdit(params.row)} sx={buttonSx}>Edit</Button>}{params.row.status === 'draft' && <Button size="small" onClick={() => publish(params.row)} sx={buttonSx}>Publish</Button>}{params.row.status === 'draft' && <Button size="small" color="error" onClick={() => remove(params.row)} sx={buttonSx}>Hapus</Button>}{params.row.status === 'published' && <Button size="small" color="warning" onClick={() => withdraw(params.row)} sx={buttonSx}>Tarik Distribusi</Button>}{['published', 'closed'].includes(params.row.status) && <Button size="small" onClick={() => createCorrection(params.row)} sx={buttonSx}>Buat Koreksi</Button>}</div> },
    ];
    ...
    const recipientColumns = [
        ...
        { field: 'download_count', headerName: 'Diunduh', width: 100, valueGetter: (value, row) => row.download_count ?? fileOf(row)?.download_count ?? 0 },
        { field: 'actions', headerName: 'Aksi', width: 260, sortable: false, renderCell: (params) => <div className="flex gap-1 flex-wrap">{canUploadRecipient(selected, params.row) && <Button component="label" size="small" variant="outlined" disabled={actionLoading} sx={{ ...buttonSx, borderColor: '#e4e4e7', color: '#3f3f46' }}>Upload<input type="file" hidden onChange={(event) => uploadRecipient(params.row, event.target.files?.[0] || null)} /></Button>}<Button size="small" disabled={selected?.status === 'closed' || !distributionFileId(fileOf(params.row))} onClick={() => downloadRecipientFile(params.row)} sx={buttonSx}><DownloadOutlined sx={{ fontSize: 16 }} /></Button></div> },
    ];
    ...
    const bulkStatusLabel = {
        uploaded: 'ZIP diterima, menunggu pemeriksaan',
        processing: 'Sedang memeriksa ZIP dan mencocokkan file',
        preview_ready: 'Preview siap diperiksa',
        confirming: 'Sedang menyimpan file yang cocok',
        confirmed: 'Selesai dikonfirmasi',
        failed: 'Pemeriksaan gagal',
        expired: 'Preview kedaluwarsa',
