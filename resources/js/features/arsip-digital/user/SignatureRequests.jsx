...
function StudentRequests() {
    ...
    const load = async () => {
        try {
            const [requests, available, files, settings] = await Promise.all([arsipApi.signatureRequests(), arsipApi.signatureRequestLecturers(), arsipApi.files({ per_page: 100 }), arsipApi.signatureRequestConfig()]);
            const requestData = payloadOf(requests); const fileData = payloadOf(files); const settingData = payloadOf(settings);
            setItems(requestData.data ?? []);
            setLecturers(payloadOf(available));
            ...
            setArchives((fileData.files ?? fileData.data ?? []).filter((file) => nameOf(file).toLowerCase().endsWith('.pdf')));
    ...
    return <div><PageHeader title="Request Tanda Tangan" subtitle="Kirim PDF dari Arsip Saya kepada dosen untuk ditandatangani." actions={<Button variant="contained" startIcon={<AddOutlined />} onClick={() => showForm()}>Buat Request</Button>} />
        ...
        <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm"><DialogTitle>{editing ? 'Edit Request' : 'Buat Request'}</DialogTitle><DialogContent className="space-y-4 !pt-2"><TextField select fullWidth label="Dosen" value={form.lecturer_user_id} disabled={!!editing} onChange={(e) => setForm({ ...form, lecturer_user_id: e.target.value })}>{lecturers.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField><TextField fullWidth label="Judul" value={form.title} inputProps={{ maxLength: 150 }} onChange={(e) => setForm({ ...form, title: e.target.value })} /><TextField fullWidth multiline minRows={3} label="Deskripsi" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><TextField select fullWidth SelectProps={{ multiple: true }} label="PDF dari Arsip Saya" value={form.file_ids} helperText={`Maks ${limits.maxFiles} file, ${limits.maxFileMb} MB/file, ${limits.maxTotalMb} MB total`} onChange={(e) => setForm({ ...form, file_ids: e.target.value.slice(0, limits.maxFiles) })}>{archives.map((file) => <MenuItem key={fileIdOf(file)} value={fileIdOf(file)}><Checkbox checked={form.file_ids.includes(fileIdOf(file))} />{nameOf(file)}</MenuItem>)}</TextField></DialogContent><DialogActions><Button onClick={() => setOpen(false)}>Batal</Button><Button variant="contained" disabled={!form.lecturer_user_id || !form.title.trim() || !form.file_ids.length} onClick={save}>Simpan</Button></DialogActions></Dialog>
    </div>;
}

function LecturerRequests() {
    const navigate = useNavigate();
    const [items, setItems] = useState([]); const [selected, setSelected] = useState([]); const [available, setAvailable] = useState(false); const [page, setPage] = useState(1); const [pages, setPages] = useState(1); const [status, setStatus] = useState(''); const [reason, setReason] = useState(''); const [action, setAction] = useState(''); const [error, setError] = useState('');
    const load = async () => { try { const response = payloadOf(await arsipApi.signatureRequests({ page, ...(status ? { status } : {}) })); setItems(response.data ?? []); setPages(response.last_page ?? 1); setSelected([]); setError(''); } catch (err) { setError((await formatArsipError(err)).message); } };
    useEffect(() => { load(); }, [page, status]);
    useEffect(() => { arsipApi.signatureRequestAvailability().then((response) => { setAvailable(Boolean(payloadOf(response).is_available)); setError(''); }).catch(async (err) => setError((await formatArsipError(err)).message)); }, []);
    const eligible = items.filter((item) => item.status === 'requested').map(idOf); const all = eligible.length > 0 && eligible.every((id) => selected.includes(id));
